...
async function getPag1(pokeApiLinkParam) {
    ...
    for (let index = 0; index < response.data.results.length; index++) {
        ...
        getElementById(pokemonId).innerHTML += '<div class="col-xl-2 col-md-3 col-sm-4 col-6 mb-4"  >' +
            '<div value="' + index + '"><img src="' + link + '"  alt="' + element.name + '">' +
            '<h6 value="' + index + '">' + element.name + '</h6></div>' +
            ...
            if(getTeamStorage().length==6){
                document.getElementsByClassName('btn btn-success')[index].innerHTML = null
...
é o link de todos eles*/
    ...
    // getElementById(btnAddPokeId).addEventListener('click', function(event){
